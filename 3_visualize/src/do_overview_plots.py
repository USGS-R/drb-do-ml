...
axs = df_daily_means.plot(subplots=True, figsize=(12, 24), sharey=True, grid=True, layout=(n_rows, 2), color='steelblue')
...
axs = inst_daily_means.plot(subplots=True, figsize=(12, 24), sharey=True, grid=True, layout=(n_rows, 2), color='steelblue')
for ax in axs.flatten():
    ax.legend(loc='upper left')
fig = plt.gcf()
...
doy_means = inst_doy_means.join(daily_doy_means, lsuffix='_inst', rsuffix='_daily')
...
axs = df_daily_means_filt.plot(subplots=True, figsize=(6, 10), sharey=True, grid=True, color='steelblue')
for ax in axs.flatten():
    ax.legend(loc='upper left')
fig = plt.gcf()
fig.suptitle('daily mean DO (mg/l) at daily sites', y=0.99)
...
axs = df_inst_means_filt.plot(subplots=True, figsize=(8, 10), sharey=True, grid=True, color='steelblue', layout=(6, 2), alpha=0.6)
